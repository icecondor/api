...
/* record functions */
  ...
  dblib.update_user_premium = function(id, premium_date) {
    return db.then(function(conn){
      return rethink.table('users').get(id).run(conn).then(function(user){
        console.log('update_user_premium', id, user)
        user.update({level: {premium: premium_date}}).run(conn)
      })
    })
  }

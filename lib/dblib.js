...
exports.factory = function(rethink, db) {
  ...
  dblib.activity_count = function(start, stop, type, distinct_user) {
    return db.then(function(conn){
      var table = rethink.table('activities')
      if(start && stop) {
        table = table.between(start.toISOString(), stop.toISOString(), {index: 'date',
                                                                        left_bound:'open',
                                                                        right_bound:'closed'})
      }
      if(type) {
        table = table.filter(rethink.row('type').eq(type))
      }
      if(distinct_user){
        table = table("user_id").distinct()
      }
      return table.count().run(conn)
    })
  }
    ...
    return db.then(function(conn){
      return rethink.table('users').filter(terms).run(conn).then(function(cursor){
        return cursor.toArray().then(function(users){
          if(users.length == 1) {
            var user = users[0]
            console.dir({id:user.id, username: user.username, email: user.email})
            return user
          }

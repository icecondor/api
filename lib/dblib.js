...
/* record functions */
  ...
  dblib.fence_update = function(record) {
    console.log('fence_update', record)
    return db.then(function(conn){
      return rethink.table('fences').get(record.id).update(record).run(conn)
    })
  }
  ...
  dblib.fences_intersect = function(polygon) {
    return db.then(function(conn){
      return rethink.table('fences').getIntersecting(polygon, {index: 'geojson'}).run(conn)
    })
  }

...
/* record functions */
  ...
  dblib.user_add_friend_request = function(user_id, friend_id) {
    console.log('db.user_add_friend_request', user_id, friend_id)
    return db.then(function(conn){
      return rethink.table('users').get(user_id).
                 update({friend_requests:rethink.row('friend_requests').prepend(friend_id)}).run(conn)
    })
  }
  ...
  dblib.user_add_friend = function(user_id, friend_id) {
    ...
    return db.then(function(conn){
      // remove request
      return rethink.table('users').get(user_id).
                 update({friend_requests:rethink.row('friend_requests').setDifference([friend_id])}).run(conn)
      return rethink.table('users').get(friend_id).
                 update({friends:rethink.row('friends').prepend(user_id)}).run(conn)
    })
  }

...
exports.factory = function(rethink, db) {
  ...
  var schema = { 'users': {indexes: ['username']},
                 'activities': {indexes: ['date', 'user_id',
                    ['date_user_id', [rethink.row("date"), rethink.row("user_id")]] ]}}
  ...
  function ensure_index(conn, table_name, index_desc) {
    return rethink.table(table_name).indexList().run(conn).then(function(indexes){
      var index_name = index_desc
      if(Array.isArray(index_desc)) {
        index_name = index_desc[0]
      }
      ...
      if(indexes.indexOf(index_name) == -1) {
        ...
        var build_index
        if(Array.isArray(index_desc)) {
          build_index = table.indexCreate(index_desc[0], index_desc[1])
        } else {
          build_index = table.indexCreate(index_name)
        }
        console.log("Warning: building table "+table_name+" index "+index_name)
        return build_index.run(conn).then(function(){
  ...
  function build_find_query(user_id, start, stop, count, type, order) {
    ...
    console.log('adjusted between '+start+' and '+stop)
    query = query.between([start.toISOString(), user_id],
                          [stop.toISOString() , user_id],
                                    {index: "date_user_id",
                                     left_bound:'open',
                                     right_bound:'closed'})
    var filters = []

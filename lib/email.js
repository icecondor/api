...
exports.factory = function(config) {
  ...
  o.build_fence_alert_email = function(email, fence_name, username, direction) {
    console.log('build_dump_email for', email)
    var emailOpt = {
        from: config.from,
        to: email,
        subject: fence_name+" was "+direction+" by "+username
      }
    var templateFile = 'email/fence_alert.jade'
    var user_link = 'https://icecondor.com/' + username
    var templateOpts = {username: username, user_link: user_link, fence_name: fence_name}
    emailOpt.html = jade.compileFile(templateFile, {pretty: true})(templateOpts)
    return emailOpt
  }
